...
void JoinClause::dump(WriteBuffer & buffer) const
{
    ...
    if (!additional_mixed_join_conditions.empty())
        buffer << " mixed_condition_nodes: " + dump_dag_nodes(additional_mixed_join_conditions);
...
std::unordered_set<JoinTableSide> extractJoinTableSidesFromExpression(const ActionsDAG::Node * expression_root_node,
...
{
    std::unordered_set<JoinTableSide> table_sides;
...
static void getFunctionArguments(const ActionsDAG::NodeRawConstPtrs & children, NamesAndTypesList & column_names)
{
    ...
    for (size_t i = 0; i < num_arguments; ++i)
    {
        ...
        if (child.type == ActionsDAG::ActionType::COLUMN || child.type == ActionsDAG::ActionType::INPUT)
        {
            column_names.push_back({child.result_name, child.result_type});
        }
        else if (child.type == ActionsDAG::ActionType::FUNCTION)
        {
            getFunctionArguments(child.children, column_names);
        }
        else
        {
            throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "{} is not supported in JOIN ON expression", child.type);
        }
    }
}

static NamesAndTypesList getFunctionArguments(const ActionsDAG::NodeRawConstPtrs & children)
{
    NamesAndTypesList column_names;
    getFunctionArguments(children, column_names);
    return column_names;
}
    ...
    JoinClause & join_clause)
{
    ...
    if (function_name == "equals" || is_asof_join_inequality)
    {
        ...
        {
            join_clause.addCondition(left_expression_sides.front(), join_expressions_actions_node);
        }
        else if (left_expression_sides.empty() && !right_expression_sides.empty())
        {
            join_clause.addCondition(right_expression_sides.front(), join_expressions_actions_node);
        }
        else
        {
            auto left_expression_side = left_expression_sides.front();
            auto right_expression_side = right_expression_sides.front();
    ...
    if (expression_sides.empty())
        ...
        join_clause.addCondition(expression_sides.front(), join_expressions_actions_node);
    else
